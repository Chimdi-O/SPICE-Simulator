...
### Current state 
- The program currently has Resistors, Inductors, Capacitors, Voltage Sources and Current Sources implemented and operating point analysis 
- certain strange circuit configurations don't simulate planned to solved that using super nodes.  

## 2nd Sep 2026

### Current state 
- part way through implmenting transient analysis to the program
- Currently the transient models for capacitors and inductors have been and I am halfway through creating the transient class 
...
- problem with the ringing of a RLC circuit where the plot was far too flat and almost like a square wave which turned out to be because I was rounding the voltage values in parseresultsMatrix() function instead of the printvalues() function in the circuit class. 
...
### Current state 
- implemented diodes 
    - Created a new diode class 
    - Integrated diodes into the interpreter
    - Updated the operating point and transient class to use newton raphson to solve the MNA matrix when a diode is present
- slightly improved the output formatting when displaying the nodes and components in transient analysis

### todo 
- Have newton raphson be in a parent class which both operating point and transient inherit from or it should be in another file and should just be callable
